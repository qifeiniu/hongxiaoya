    ...
    <view class="header-section">
      ...
      <view class="custom-nav" :style="{ paddingTop: statusBarHeight + 'px' }">
        <view class="nav-content">
          ...
          <text class="nav-title">{{ pageType === 'egg' ? '鸭蛋明细' : '钱包明细' }}</text>
        </view>
      </view>
      <view class="header-banner" :style="{ marginTop: `calc(${statusBarHeight}px + 44px)` }">
        <view class="commission-info" v-if="pageType === 'egg'">
          <text class="balance-title">鸭蛋总余额</text>
          <view class="commission-row">
            <text class="balance-num commission-num">{{ totalBalance }}</text>
            <view class="withdraw-btn" @click="handleRecharge">
              <text>去充值</text>
              <text class="ri-arrow-right-s-line"></text>
            </view>
          </view>
        </view>
        <view class="commission-info" v-else-if="isMatchmaker">
      ...
      <scroll-view scroll-y class="list-container">
        ...
        <view class="list-item" v-for="(item, index) in filteredList" :key="index">
          ...
          <view class="item-right">
            <template v-if="pageType !== 'egg' && (item.type === 'recharge_rmb' || (item.rmbAmount && item.rmbAmount < 0))">
              <text class="item-amount expense">{{ item.rmbAmount }}元</text>
            </template>
            <template v-else-if="pageType !== 'egg' && (item.rmbAmount && item.rmbAmount > 0)">
              <text class="item-amount income">+{{ item.rmbAmount }}元</text>
            </template>
            <template v-else>
              <text class="item-amount" :class="item.amount > 0 ? 'income' : 'expense'">
...
const filteredList = computed(() => {
  ...
  if (pageType.value === 'egg') {
    // 鸭蛋明细: 过滤出与鸭蛋有关的记录
    list = list.filter(item => typeof item.amount === 'number' && item.amount !== 0)
